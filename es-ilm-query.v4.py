...
for idx in indices:
    ...
    if size_bytes < size_threshold:
        ...
        # Check ILM info using get_lifecycle
        ilm_info = es.ilm.get_lifecycle(index=index_name)
        index_ilm = ilm_info.get(index_name, {})
        
        if index_ilm.get("policy"):  # Check if ILM policy exists
            policy = index_ilm["policy"]["name"]
