...
# Fetch ILM policy details
policy_settings = {}
for policy in groups.keys():
    try:
        # Verify if policy exists
        policy_info = es.ilm.get_lifecycle(policy=policy)
        ...
        for phase, config in phases_def.items():
            ...
            rollover = actions.get('rollover', {})
            if rollover:
                rollover_settings[phase] = rollover
            else:
                rollover_settings[phase] = {"note": "No rollover settings defined"}
        
        if not rollover_settings:
            print(f"Warning: No rollover settings found for policy '{policy}'")
            policy_settings[policy] = {"note": "No phases with rollover settings"}
        else:
            policy_settings[policy] = rollover_settings
    except Exception as e:
        print(f"Error: Failed to get ILM policy details for '{policy}': {str(e)}")
